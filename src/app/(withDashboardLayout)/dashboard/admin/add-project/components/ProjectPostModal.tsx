...
const ProjectPostModal = ({ open, setOpen }: TProps) => {
  ...
  const techStack = useAppSelector(
    (state: RootState) => state.addTechnology.techStack
  );
  ...
  const handleSubmit = async (values: FieldValues) => {
    ...
    if (values?.file) {
      const res = await fetch(
        "https://api.imgbb.com/1/upload?key=911aa569c7b137d0c46ecda476b6c122",
        {
          method: "POST",
          body: formData,
        }
    ...
    <MYFullScreenModal open={open} setOpen={setOpen} title="Add a new project">
      <MyForm onSubmit={handleSubmit}>
        <Grid container spacing={2}>
          ...
          <Grid item md={6} sm={12} xs={12}>
            <MYInput
              name="backendSiteCode"
              label="Backend Site Code Link"
              type="text"
              fullWidth={true}
              required={true}
            />
